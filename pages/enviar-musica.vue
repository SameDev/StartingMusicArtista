      ...
      <div class="container justify-center items-center content-center mx-auto bg-base-300 w-full shadow-xl p-7 m-10 rounded-2xl font-nunito">
        <div>
          ...
          <form @submit.prevent="enviarMusica">
            ...
            <div class="mb-4">
              <label for="duracao" class="block text-white font-bold text-sm mb-2">Duração:</label>
              <input type="time"  id="duracao" v-model="duracao" class="input input-bordered bg-accent text-white w-full" placeholder="mm:ss" required>
            </div>
            ...
            <Success v-if="success" success-message="Música enviada com sucesso!"/>
...
export default {
  ...
  methods: {
    ...
    verificaCampos() {
      if (!this.date || !this.nome || !this.artista || !this.duracao) {
        this.error = true;
